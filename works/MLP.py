...
class Relu():      #relu?Dead Relu!  
    def forward(self, x):# x->1*size ndarray
        self.y=(x>0)*x
        return self.y
        """
...
class Softmax_Cross_Entropy():
    def forward(self, x):#x ->1d ndarray
        """
            TODO: Finish the forward propagation function and save some variables if you need
        """
        self.y=np.exp(x)
        self.sum=np.sum(self.y)
        self.y=self.y/self.sum
        return self.y
    def backward(self, label):#Todo
        """
            TODO: Finish the backward propagation function 
        """        
        return self.y-label
...
class Linear(Layer):    
    ...
    def backward(self, node_grad):
        ...
        """
        self.grad=np.dot(self.x.T,node_grad)
        self.gradb=node_grad
